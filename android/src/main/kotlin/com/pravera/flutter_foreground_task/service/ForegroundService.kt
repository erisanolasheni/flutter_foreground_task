...
class ForegroundService : Service(), MethodChannel.MethodCallHandler {
    companion object {
        ...
        private const val ACTION_TASK_REMOVED = "onTaskRemoved"
    ...
    override fun onDestroy() {
        super.onDestroy()
        Log.i(TAG, "FFG: On destroy called.")
        stopForegroundTask()
        stopForegroundService()
    ...
    override fun onTaskRemoved(rootIntent: Intent?) {
        Log.i(TAG, "FFG: On Task removed called.")
        super.onTaskRemoved(rootIntent)
		backgroundChannel?.invokeMethod(ACTION_TASK_REMOVED, null)
    }
    ...
    private fun getTextSpan(text: String, color: Int?): Spannable {
        return if (color != null) {
            SpannableString(text).apply { setSpan(ForegroundColorSpan(color), 0, length, 0) }
        } else {
    ...
    private fun buildButtonActions(): List<Notification.Action> {
        ...
        for (i in buttons.indices) {
            val bIntent =
                    Intent(ACTION_NOTIFICATION_BUTTON_PRESSED).apply {
